    ...
    <div class="col-12 col-md-6 form-panel column items-center justify-center q-pa-md">
      <q-card flat class="login-card">
        ...
        <q-card-section>
          ...
          <q-form @submit.prevent="handleLogin" class="q-gutter-md">
            <q-input v-model="form.identifier" outlined label="Email" lazy-rules :rules="[
              (val) => !!val || 'Wajib diisi',
      ...
      <div class="text-caption text-grey-7 lt-md q-mt-xs text-center">
        <span class="text-bold">Made with <q-icon name="favorite" color="red-10" size="xs" class="q-mx-xs" /> by.
          Deden
          Moh Jaenudin.</span>
        <span class="block"> © {{ currentYear }} Hak cipta dilindungi.</span>
      </div>
    </div>
  </div>
    ...
    <q-card class="reset-dialog">
      <q-card-section>
        ...
        <div class="text-caption text-grey-7 q-mt-xs">
          Masukkan alamat email. Permintaan reset akan diteruskan ke Operator SIAKAD untuk direset
          manual.
        </div>
      </q-card-section>
      <q-separator />
...
const handleLogin = async () => {
  ...
  try {
    ...
    console.log(err)
    errorMessage.value = error.value || 'Terjadi kesalahan pada server.'
  }
}
