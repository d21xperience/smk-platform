      ...
      <q-card-section class="profile-banner__content">
        <div class="row items-end q-col-gutter-md">
          ...
          <div class="col">
            ...
            <div class="text-body2 text-grey-3">{{ profile.jabatan }} &middot; {{ profile.unitKerja }}</div>
          ...
          <div class="col-auto gt-xs">
            <q-btn no-caps outline color="white" icon="edit" label="Edit Profil" @click="showEditNotice" />
          </div>
        </div>
      </q-card-section>
    </q-card>
    ...
    <div class="row q-col-gutter-md">
      ...
      <div class="col-12 col-md-4">
        ...
        <div class="q-mt-md" v-if="documentAlerts.length">
          ...
          <q-card flat bordered class="alert-card q-mb-sm" v-for="alert in documentAlerts" :key="alert.id">
          ...
          <q-card-section>
            <div class="row items-center justify-between q-mb-md wrap">
              ...
              <div class="row q-gutter-sm items-center">
                <q-btn no-caps unelevated color="primary" icon="upload_file" label="Unggah Berkas"
            ...
            <div class="row q-col-gutter-sm q-mb-md">
              <div class="col-12 col-sm-6">
                <q-input dense outlined v-model="searchQuery" placeholder="Cari nama berkas..." clearable>
              ...
              <div class="col-12 col-sm-6">
                <q-select dense outlined v-model="filterType" :options="typeFilterOptions" emit-value map-options
                  label="Jenis Berkas" />
              </div>
            </div>
            ...
            <q-list v-if="filteredYears.length" bordered class="rounded-borders">
              <q-expansion-item v-for="group in filteredYears" :key="group.year" :label="`Tahun ${group.year}`"
                :caption="`${group.files.length} berkas`" header-class="year-header text-weight-bold" default-opened
                icon="folder">
                <q-separator />
                <q-list separator>
                  <q-item v-for="file in group.files" :key="file.id">
                    <q-item-section avatar>
                      <q-icon :name="fileIcon(file.ext)" :color="fileIconColor(file.ext)" size="32px" />
                    </q-item-section>

                    <q-item-section>
                      <q-item-label class="text-weight-medium">{{ file.name }}</q-item-label>
                      <q-item-label caption>
                        {{ file.type }} &middot; {{ file.size }} &middot; Diunggah {{ file.uploadedAt }}
                      </q-item-label>
                    </q-item-section>
                    ...
                    <q-item-section side>
                      <q-btn flat round dense icon="download" color="primary" @click="handleDownload(file)">
